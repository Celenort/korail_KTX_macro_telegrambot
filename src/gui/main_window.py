...
class MainWindow(QMainWindow):
    ...
    def _reservation_tab(self) -> QWidget:
        ...
        self.source_field = QWidget(); source_layout = QHBoxLayout(self.source_field)
        source_layout.setContentsMargins(0, 0, 0, 0)
        source_layout.addWidget(QLabel("출발역")); source_layout.addWidget(self.source, 1)
        self.destination_field = QWidget(); destination_layout = QHBoxLayout(self.destination_field)
        destination_layout.setContentsMargins(0, 0, 0, 0)
        destination_layout.addWidget(QLabel("도착역")); destination_layout.addWidget(self.destination, 1)
        station_row.addWidget(self.source_field, 1); station_row.addWidget(self.destination_field, 1)
        self.time_fields = QWidget()
        time_row = QHBoxLayout(self.time_fields); time_row.setContentsMargins(0, 0, 0, 0)
        self.after_field = QWidget(); after_layout = QHBoxLayout(self.after_field)
        after_layout.setContentsMargins(0, 0, 0, 0)
        after_layout.addWidget(self.after, 1); after_layout.addWidget(QLabel("부터"))
        self.before_field = QWidget(); before_layout = QHBoxLayout(self.before_field)
